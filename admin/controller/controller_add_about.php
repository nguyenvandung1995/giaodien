<?php
	...
	switch($act){
		case'add':
			$form_control="index.php?controller=add_about&act=do_add";
			break;
		case'do_add':
			if($_SERVER["REQUEST_METHOD"] == "POST"){
				$c_name = htmlspecialchars($_POST["c_name"],ENT_QUOTES);
				$c_masp = htmlspecialchars($_POST["c_masp"],ENT_QUOTES);
				$fk_category_about = htmlspecialchars($_POST["fk_category_about"],ENT_QUOTES);	
				...
				$c_date_created=date("y/m/d");
	 			...
	 			//if(isset($_FILES["link_img"]) && $_FILES["link_img"]["name"] != ""){
					...
					//echo "insert into tbl_about(c_name,c_masp,fk_category_about_id,c_color,c_nguongoc,c_chatlieu,c_price,c_description,c_noidung,c_img,c_date_created,c_sex,link_img) values('$c_name','$c_masp',$fk_category_about,'$c_color','$c_nguongoc','$c_chatlieu',$c_price,'$c_desription','$c_noidung','$c_img','$c_date_created','$c_sex','$link_img')";
				...
				$ckeck=fetch_one("select * from tbl_about where c_masp='$c_masp'");
				if($ckeck>0){
					echo "<div style='text-align:center;color:RED;font-size:25px'>Mã sản phẩm: ".$c_masp." đã tồn tại</div>";
					$form_control="index.php?controller=add_about&act=do_add";
				}
				else{
					execute("insert into tbl_about(c_name,c_masp,fk_category_about_id,c_color,c_nguongoc,c_chatlieu,c_price,c_description,c_noidung,c_img,c_date_created,c_sex,c_sanphamnoibat) values('$c_name','$c_masp',$fk_category_about,'$c_color','$c_nguongoc','$c_chatlieu',$c_price,'$c_desription','$c_noidung','$c_img','$c_date_created','$c_sex',$c_sanphamnoibat)");
	...
	//
	include_once("view/view_add_about.php");
?>